...
function optionsframework_add_admin() {
	$of_page = add_menu_page( 'Fenicia Theme Options', 'Theme Options', 'activate_plugins', 'optionsframework', 'optionsframework_options_page', '
dashicons-admin-generic');
	//$of_page = add_theme_page( THEMENAME, __( 'FMR Theme Options', 'zenwonderss' ), 'edit_theme_options', 'optionsframework', 'optionsframework_options_page');
...
function of_ajax_callback() {
	...
	if($save_type == 'upload') {
		...
		if(!empty($uploaded_file['error'])) {
			echo __('Upload Error: ','cleangold') . $uploaded_file['error'];
		} else {
...
?>
	<script type="text/javascript">
		jQuery(function(){
			jQuery('.themes-php #message2').html('<p><?php printf( __( "This theme comes with an %s to configure settings. It is recommended you also visit the %s and %s.", 'cleangold' ), '<a href="' . admin_url('themes.php?page=optionsframework') .'">' . __('options panel', 'cleangold') . '</a>', '<a href="' . admin_url('widgets.php') .'">' . __('widgets editor', 'cleangold') . '</a>','<a href="' . admin_url('nav-menus.php') .'">' . __('menu editor', 'cleangold') . '</a>' ); ?></p>');
		});
	</script>
<?php
}

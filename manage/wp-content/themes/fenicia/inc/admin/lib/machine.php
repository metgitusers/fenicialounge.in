...
class Options_Machine {
		...
		foreach ($options as $value) {
			...
			switch ( $value['type'] ) {
				...
				case 'slider': // drag & drop slide manager
					...
					$output .= '</ul>';
					$output .= '<a href="#" class="button slide_add_button">' . __('Add New Slide','cleangold') . '</a></div>';
					break;
				...
				case 'backup': // backup and restore options data
					...
					if(!isset($backup['backup_log'])) {
						$log = __('No backups yet', 'cleangold');
					} else {
						$log = $backup['backup_log'];
					}
					$output .= '<div class="backup-box">';
					$output .= '<div class="instructions">'.$instructions."\n";
					$output .= '<p><strong>' . __('Last Backup','cleangold') . ' : <span class="backup-log">' . $log . '</span></strong></p></div>'."\n";
					$output .= '<a href="#" id="of_backup_button" class="button">' . __('Backup Options','cleangold') . '</a>';
					$output .= '<a href="#" id="of_restore_button" class="button">' . __('Restore Options','cleangold') . '</a>';
					$output .= '</div>';
					break;
				...
				case 'transfer': // export or import data between different installs
					...
					$output .= '<a href="#" id="of_import_button" class="button">' . __('Import Options','cleangold') . '</a>';
					break;
	...
	public static function optionsframework_media_uploader_function ($id, $std, $mod) {
		...
		if ( function_exists( 'wp_enqueue_media' ) ) {
			$uploader .= '<span class="button media_upload_button" id="'.$id.'">' . __('Upload','cleangold') . '</span>';
			
			if(!empty($upload)) {$hide = '';} else { $hide = 'hide';}
			$uploader .= '<span class="button remove-image '. $hide.'" id="reset_'. $id .'" title="' . $id . '">' . __('Remove','cleangold') . '</span>';
		}
		$uploader .='</div>' . "\n";
	...
	public static function optionsframework_slider_function($id,$std,$oldorder,$order){
		...
		$slidevars = array('title','url','mainheading','subheading','shortdescription','link','description');
		...
		if (!empty($val['title'])) {
			...
			$slider .= '<li><div class="slide_header"><strong>' . __('Slide','cleangold') . ' '.$order.'</strong>';
		}
		$slider .= '<input type="hidden" class="slide of-input order" name="'. $id .'['.$order.'][order]" id="'. $id.'_'.$order .'_slide_order" value="'.$order.'" />';
		$slider .= '<a class="slide_edit_button" href="#">' . __('Edit','cleangold') . '</a></div>';
		$slider .= '<div class="slide_body">';
		$slider .= '<label>' . __('Title','cleangold') . '</label>';
		...
		$slider .= '<label>' . __('Image URL','cleangold') . '</label>';
		$slider .= '<input type="text" class="upload slide of-input" name="'. $id .'['.$order.'][url]" id="'. $id .'_'.$order .'_slide_url" value="'. $val['url'] .'" />';
		$slider .= '<div class="upload_button_div"><span class="button media_upload_button" id="'.$id.'_'.$order .'">' . __('Upload','cleangold') . '</span>';
		if(!empty($val['url'])) {
			$hide = '';
		} else {
			$hide = 'hide';
		}
		$slider .= '<span class="button remove-image '. $hide.'" id="reset_'. $id .'_'.$order .'" title="' . $id . '_'.$order .'">' . __('Remove','cleangold') . '</span>';
		$slider .='</div>' . "\n";
		...
		$slider .= '</div>';
		$slider .= '<label>' . __('Link URL (optional)','cleangold') . '</label>';
		$slider .= '<input type="text" class="slide of-input" name="'. $id .'['.$order.'][link]" id="'. $id .'_'.$order .'_slide_link" value="'. $val['link'] .'" />';
		$slider .= '<label>' . __('Description (optional)','cleangold') . '</label>';
		$slider .= '<textarea class="slide of-input" name="'. $id .'['.$order.'][description]" id="'. $id .'_'.$order .'_slide_description" cols="8" rows="8">'.stripslashes($val['description']).'</textarea>';
		$slider .= '<a class="slide_delete_button button-primary" href="#">' . __('Delete','cleangold') . '</a>';

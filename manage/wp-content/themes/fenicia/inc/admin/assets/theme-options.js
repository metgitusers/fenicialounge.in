...
jQuery(document).ready(function() {
	...
	jQuery('head').append('<style type="text/css">#of_container .cb-enable.selected span, #of_container .section-sliderui .ui-slider .ui-slider-range.ui-widget-header { background-color: ' + jQuery('.button-primary').css('backgroundColor') + '; border-color: ' + jQuery('.button-primary').css('borderColor') + '; } #of_container .cb-enable.selected span { text-shadow: 1px 1px 0 ' + jQuery('.button-primary').css('borderColor') + '; }</style>')
	...
	if (jQuery.cookie("of_current_opt") === null) {
		...
		jQuery.each(hooks, function(key, value) {
			if (jQuery.cookie("of_current_opt") == '#of-option-'+ value) {
				jQuery('.group#of-option-' + value).show(0);

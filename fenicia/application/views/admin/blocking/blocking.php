            ...
            <div class="card">
              <div class="card-header">
                <div class="page-title-wrap">
                  <h4 class="card-title">Zone Blocking Details</h4>                  
                </div>
                          ...
                          <div class="user_permission_top">
  	                        <div class="row">	                        	
                                ...
                                <div class="form-group">
                                  <label>Blocking Date: <sup>*</sup></label>
                                  <div class="input-group blocking_dt" id="blocking_from_date_div" style="margin-bottom:12px;">
                                    <input type="text" placeholder="DD/MM/YYYY" required name="blocking_from_date" id="blocking_from_date" value="" class="blocking_date form-control pickadate" readonly="true"/>
                                  ...
                                  <span class="error" style="margin-top:10px"></span>
                                </div>
                              </div>
                              <!--<div class="col-md-2">
                                <div class="form-group">
                              ...
                              <div class="col-md-3">
                                <div class="form-group"> 
                                  <label>Blocking Time: <sup>*</sup></label>                                         
                                  <div class="input-group time_pick blocking_dt" id="blocking_from_time_div" style="margin-bottom:12px;">                                                                            
...
$(document).on('click','#search_btn',function(){
    ...
    if(blocking_from_time == ''){
        $("#blocking_from_time_div").next('span').html('Put blocking from time');
        ctn_error++;
    }
    else{
      $("#blocking_from_time_div").next('span').html(' ');
      
    }
    if(club_zone == ''){
        $("#club_zone_name").next('span').html('Select zone');
        ctn_error++;
    }
    else{
      $("#club_zone_name").next('span').html(' ');
   ...
   /* if(blocking_to_date == ''){
    ...
    if(blocking_to_time == ''){
        $("#blocking_to_time_div").next('span').html('Put blocking to time');
        ctn_error++;
    }
    else{
      $("#blocking_to_time_div").next('span').html(' ');
    ...
    */
    if(ctn_error == 0){
      $.ajax({
          type: "POST",
          url: '<?php echo base_url('admin/Zoneblocking/blockZone')?>', 
          data:{blocking_from_date : blocking_from_date,blocking_from_time : blocking_from_time,club_zone:club_zone},
          datatype:'html',
    ...
    buttons: {            
        Okay: {
            ...
            action: function(){
              $.ajax({
                      type: "POST",
                      url: '<?php echo base_url('admin/Zoneblocking/doBlockZone')?>', 

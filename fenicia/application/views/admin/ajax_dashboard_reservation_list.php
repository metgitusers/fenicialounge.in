      ...
      <tbody>
        ...
        <tr>
            ...
            <?php if(($list['resv_status'] == 1 || $list['resv_status'] == 2) && ($list['check_in_out_status'] == 2 || $list['check_in_out_status'] == 1) ){ ?>
                    <td><a style="color: #666EE8;text-decoration: underline;" class="edit_link" data-href="<?=base_url('admin/reservation/edit/'.$list['reservation_id'].'/dashboard')?>" ><strong><?= $list['reservation_id']; ?></strong></a></td> 
            <?php }else{ ?>
                    <td><a style="text-decoration: underline;" href="<?=base_url('admin/reservation/ViewDetails/'.$list['reservation_id'])?>" ><strong><?= $list['reservation_id']; ?></strong></a></td>
            <?php  } ?>                                                                                                  
            ...
            <td class="action_bttn action_td text-center">
                <?php // if(($list['resv_status'] == 1 || $list['resv_status'] == 2) && ($list['check_in_out_status'] == 2) ){ ?>
                        <!--<a title="Edit" class="resev_edit" href="javascript:void(0);" data-href="<?=base_url('admin/reservation/edit/'.$list['reservation_id'])?>" class="edit_bttn btn_action edit_icon"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></a>-->
                <?php //} 
                      if($list['resv_status'] == 2){ ?>        
                        <?php   if($list['check_in_out_status'] =='2') { ?> 
                                  <button class="check_in_out_status ck_inout btn" data-id="<?php echo $list['reservation_id'];?>" data-href="<?=base_url('admin/reservation/checkInOutStatusChange')?>" name="check_in_out_status" value="1">Check-in</button>  
                                <!--<input type="radio"  data-id="<?php echo $list['reservation_id'];?>" data-href="<?=base_url('admin/reservation/checkInOutStatusChange')?>" name="check_in_out_status" class="check_in_out_status" value="1" <?php if($list['check_in_out_status'] == '1'){ echo "checked"; } ?>>-->
                        <?php   }
                                elseif($list['check_in_out_status'] =='1') { ?>
                                  <button class="check_in_out_status ck_inout btn" data-id="<?php echo $list['reservation_id'];?>" data-href="<?=base_url('admin/reservation/checkInOutStatusChange')?>" name="check_in_out_status" value="0">Check-out</button> 
                                  <!--<label class="ck_inout"><input type="radio"  data-id="<?php echo $list['reservation_id'];?>" data-href="<?=base_url('admin/reservation/checkInOutStatusChange')?>" name="check_in_out_status" class="check_in_out_status" value="0" <?php if($list['check_in_out_status'] == '0'){ echo "checked"; } ?>> Check-out </label>-->
                        <?php  } 
                                else{ ?>
                                <button class="ck_out btn" style="cursor: auto;" disabled><strong>Check-out</strong></button>
                                  <!--<label class="ck_inout"><input type="radio" checked readonly> Check-Out </label>-->
                          <?php }
                          ?>
                <?php } ?>
            </td>
...
$(document).ready(function() {
  ...
  /*$(document).on('click','.check_in_out_status',function(){     
...
$('button.check_in_out_status').confirm({    
    title: "confirmation!",    
    content: "Are you sure want to do this action?",  
     buttons: {
        OK: {
            btnClass: 'btn-green',
            action: function(){
              var check_in_out_status = this.$target.val();
              var reservation_id      = this.$target.data('id');

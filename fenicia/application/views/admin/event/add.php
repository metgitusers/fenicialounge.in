                  ...
                  if (empty($event_list)) { ?>
                    ...
                    <form class="form custom_form_style" method="Post" action="<?= base_url(); ?>admin/event/addEvent" enctype="multipart/form-data">
                            ...
                            <div class="col-md-2">
                              <div class="form-group">
                                ...
                                <div class="input-group time_pick">
                                  <input class="form-control timepicker" id="event_str_time" value="<?php echo set_value('event_str_time');?>" name="event_str_time" required/>
                                  <div class="input-group-append">
                                    <span class="input-group-text">
                          ...
                          <div class="row">
                            ...
                            <div class="col-md-6">
                                <label>Event Video<sup>(accept file extention - .WEBM,.MPG,.MP2,.PEG,.MPE,.MPV,.OGG,.MP4,.M4P,.M4V,.AVI,.WMV)</sup></label>
                                <div class="input-group mb-3">                              
                                  <div class="custom-file">
                                    <input type="file" accept=".webm,.mpg,.mp2,.peg,.mpe,.mpv,.ogg,.mp4,.m4p,.m4v,.avi,.wmv" name="event_img[]" class="custom-file-input" id="event_video" multiple>
                                    <label class="custom-file-label" for="inputGroupFile01">Select Video</label>
                                  </div>
                                </div>
                            </div>
                            <div class="col-md-12" id="event_video_div" style="margin-top: 27px;"></div>                                                      
                          <div class="col-md-12">
                              ...
                              <?php if($event_img_list): 
                                      foreach($event_img_list as $img_list): ?>
                                        <div style="float:left;margin-right:2px">
                                          ...
                                          <?php }else{ 
                                                  $video_filename     = explode('.',$img_list['event_img']);
                                                  $length             = count($video_filename)-1;
                                                  $extention          = $video_filename[$length]; 
                                          ?>
                                                  <video style="margin:8px" width="20%" height="30%" controls><source src="<?php echo base_url().'public/upload_image/event_image/'.$img_list['event_img'];?>" type="video/<?php echo $extention; ?>"></video>
                                                  <button class="btn btn-danger delete_pro_img" id="<?php echo $img_list['event_img_id']; ?>"><i class="fa fa-trash-o"></i></button>
                                          <?php } ?>
...
function readURL(input) {
  if (input.files) {
    ...
    for (i = 0; i < filesAmount; i++) {
        //ext[i] = input.files[i].name.substr(-3);
...
$("#event_video").change(function() {
  var ext = $('#event_video').val().split('.').pop().toLowerCase();
...
function readVideoURL(input) {
  if (input.files) {
    var filesAmount = input.files.length;
    $('#event_video_div').html("");
    for (i = 0; i < filesAmount; i++) {
        //ext[i] = input.files[i].name.substr(-3);
        var reader = new FileReader();
        reader.onload = function(event) {                     
          $($.parseHTML('<video width="13%" height="100" controls></video>')).attr('src', event.target.result).appendTo('#event_video_div');
        }
        reader.readAsDataURL(input.files[i]);
    }
  }

...
class Mmember extends CI_Model {
	...
	public function getMemberDetails($condition = null){
        $result=array();
        $query = "select mu.*,CONCAT(mu.title,' ',mu.first_name,' ',ifnull(mu.middle_name,''),' ',mu.last_name) as full_name from master_member mu where mu.is_delete = '0'".$condition." order by mu.member_id desc";
       //echo $query;exit;
        $query1 = $this->db->query($query);

...
class Mreservationcomission extends CI_Model {
	...
	public function get_reservation_list($condition = NULL){
        $result=array();
        if(!empty($condition)){
            $query = "select reservation.*,ml.zone_name,reservation.status as resv_status,CONCAT(reservation.first_name,' ',reservation.last_name) as full_name from reservation left join master_zone ml on reservation.zone_id = ml.zone_id where ".$condition." order by reservation.reservation_date desc";  
        }
        else{
            $query = "select reservation.*,ml.zone_name,reservation.status as resv_status,CONCAT(reservation.first_name,' ',reservation.last_name) as full_name from reservation left join master_zone ml on reservation.zone_id = ml.zone_id order by reservation.reservation_date desc";  
        }
     //echo $query;exit;
        $query1 = $this->db->query($query);
        $result=$query1->result_array();
    ...
    /*public function reservationCommissionFilterSearch($condition = null,$zone_id = null){    
        ...
        if(!empty($condition)){
            ...
            $query = "SELECT rev.reservation_date FROM reservation as rev left join master_zone as zone on zone.zone_id = rev.zone_id where ".$condition." and rev.status in(2,3) group by rev.reservation_date order by rev.reservation_id desc";
        }
        else{
            ...
            $query = "SELECT rev.reservation_date FROM reservation as rev left join master_zone as zone on zone.zone_id = rev.zone_id where rev.status in(2,3) group by rev.reservation_date order by rev.reservation_id desc"; 
        }
     //echo $query;exit;
        $query1  = $this->db->query($query);
        $result = $query1->result_array();
        if(!empty($result)){
            foreach($result as $val){                
                if(!empty($val['reservation_date'])){
                    ...
                    //$query = "SELECT rev.reservation_date,count(rev.reservation_id) as no_of_reservation,SUM(rpt.payment_amount) as total_price FROM reservation as rev left join reservation_payment_transaction as rpt on rpt.reservation_id = rev.reservation_id where ".$condition." group by rev.reservation_date"; 
                    //$query = "SELECT rev.reservation_date,count(rev.reservation_id) as no_of_reservation,zone.zone_name FROM reservation as rev left join master_zone as zone on zone.zone_id = rev.zone_id where ".$condition." group by rev.reservation_date"; 
                    $query_rev = "SELECT rev.reservation_date,rev.zone_id,count(rev.reservation_id) as no_of_reservation,zone.zone_name  FROM reservation as rev left join master_zone as zone on zone.zone_id = rev.zone_id where ".$cond." and rev.status in(2,3) group by rev.zone_id order by rev.reservation_id desc";
                }
                else{
                    //$query = "SELECT rev.reservation_date,count(rev.reservation_id) as no_of_reservation,SUM(rpt.payment_amount) as total_price FROM reservation as rev left join reservation_payment_transaction as rpt on rpt.reservation_id = rev.reservation_id group by rev.reservation_date";
                    $query_rev = "SELECT rev.reservation_date,rev.zone_id,count(rev.reservation_id) as no_of_reservation,zone.zone_name  FROM reservation as rev left join master_zone as zone on zone.zone_id = rev.zone_id where rev.status in(2,3) group by rev.zone_id order by rev.reservation_id desc"; 
                }
             //echo $query_rev;
    ...
    public function reservationCommissionFilterSearch($condition = null,$zone_id = null){    
        ...
        if(!empty($condition)){
            //$query = "SELECT rev.reservation_date,count(rev.reservation_id) as no_of_reservation,SUM(rpt.payment_amount) as total_price FROM reservation as rev left join reservation_payment_transaction as rpt on rpt.reservation_id = rev.reservation_id where ".$condition." group by rev.reservation_date "; 
            //$query = "SELECT rev.reservation_date,count(rev.reservation_id) as no_of_reservation,zone.zone_name FROM reservation as rev left join master_zone as zone on zone.zone_id = rev.zone_id where ".$condition." group by rev.reservation_date"; 
            $query = "SELECT rev.reservation_date,rev.zone_id,count(rev.reservation_id) as no_of_reservation,zone.zone_name  FROM reservation as rev left join master_zone as zone on zone.zone_id = rev.zone_id where ".$condition." and rev.status in(2,3) group by rev.zone_id order by zone.zone_name Asc";
        }
        else{
            //$query = "SELECT rev.reservation_date,count(rev.reservation_id) as no_of_reservation,SUM(rpt.payment_amount) as total_price FROM reservation as rev left join reservation_payment_transaction as rpt on rpt.reservation_id = rev.reservation_id group by rev.reservation_date";
            $query = "SELECT rev.reservation_date FROM reservation as rev left join master_zone as zone on zone.zone_id = rev.zone_id where rev.status in(2,3) group by rev.zone_id order by zone.zone_name Asc"; 
        }
     //echo $query;exit;
        $query1     = $this->db->query($query);
        $result     = $query1->result_array();

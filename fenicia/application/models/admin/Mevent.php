...
class Mevent extends CI_Model {
    ...
    public function get_past_event_list($status = null){
        $result=array();
        if($status !=""){
            $query = "select me.* from master_event me where me.status = '".$status."' and CURDATE() > me.event_start_date and me.is_delete = '0' order by me.event_start_date asc";
        }
        else{
            $query = "select me.* from master_event me where me.is_delete = '0' and CURDATE() > event_start_date order by me.event_start_date asc";
        }
        //echo $query;exit;
        $query1 = $this->db->query($query);
        $result=$query1->result_array();
    ...
    public function get_filter_past_event_list($condition,$status = null){
        $result=array();
        $query = "select me.* from master_event me where me.status = '".$status."' ".$condition." and CURDATE() > me.event_start_date and me.is_delete = '0' order by me.event_start_date desc";
        //echo $query;exit;
        $query1 = $this->db->query($query);
        $result=$query1->result_array();
        return $result;
    ...
    public function getEventNameByImageId($image_id){

        $result=array();
        $event_name ='';
        $query  = "select me.event_name from event_images ei left join master_event me on me.event_id = ei.event_id where ei.event_img_id ='".$image_id."'";
        $query1 = $this->db->query($query);

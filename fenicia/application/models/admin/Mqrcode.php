...
public function get_data_list(){
    ...
    $this->db->select('qrcode_data.*,master_member.first_name,master_member.last_name,master_member.mobile');
    $this->db->from('qrcode_data');
     $this->db->join('master_member', 'master_member.member_id = qrcode_data.member_id', 'left'); 
    // if(!empty($from_dt) && !empty($to_dt) ){
    // $this->db->where('comment_card.visit_date between "'.$from_dt.'" and "'.$to_dt.'"');
    // }
    $this->db->order_by("qrcode_data.id", "desc");
    $query=$this->db->get();

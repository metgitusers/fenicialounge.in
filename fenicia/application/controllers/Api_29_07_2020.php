...
class Api extends CI_Controller
{
  ...
  public function signup()
  {
   ...
   $result = $this->mapi->checkUserRegistered(array('email' => $ap['email']));
          if (!empty($result)) {
            $response['status']['error_code'] = 1;
        ...
        if (!empty($ap['mobile'])) {
          ...
          $result = $this->mapi->checkUserRegistered(array('mobile' => $ap['mobile']));
          if (!empty($result)) {
            $response['status']['error_code'] = 1;
       ...
       /* if(!empty($_FILES['profile_img']['name'])){
        ...
        if($ap['doa'] !=''){
          $doa              = date('Y-m-d',strtotime(str_replace('/','-',$ap['doa'])));
        ...
        $data = array(    
          ...
          'mobile'                => $ap['mobile'],
          'password'              => sha1($ap['password']),
          'original_password'     => $ap['password'],
          'email'                 => $ap['email'], 
          'gender'                => $ap['gender'],
          'marriage_status'       => strtolower($ap['marriage_status']),
          'dob'                   => date('Y-m-d',strtotime(str_replace('/','-',$ap['dob']))),
          'doa'                   => $doa,
          'profile_img'           => '',
          'status'                => '1',
        ...
        if($member_id)
          {
            ...
            $update_arr = array('created_by' => $member_id);
            $this->mapi->update('master_member',$condition,$update_arr); 

            $signup = $this->mapi->getRow('master_member',array('member_id' => $member_id));
            ...
            $signup['profile_pic_updated']  = '0';
            $response['status']['error_code'] = 0;
            $response['status']['message']    = 'Registration Successful.';
            $response['response']['member']   = $signup;
  ...
  /************************* Send password to the member ****************************/
            ...
            $message .=  "User ID: ".$ap['email']."\n Team Club Fenicia";
            $this->smsSend($ap['mobile'],$message);
        }
      }
      else {
          $response['status']['error_code'] = 1;
  ...
  public function smsSend2($mobile,$message){
    //echo $mobile."<br>".$message;exit;
   // $api_key = '45DB969F6550A9';
    $api_key = '45DA414F762394';
    //$contacts = '97656XXXXX,97612XXXXX,76012XXXXX,80012XXXXX,89456XXXXX,88010XXXXX,98442XXXXX';

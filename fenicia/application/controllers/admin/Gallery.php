...
class Gallery extends MY_Controller {
	...
	*/
	...
	public function UpdateGallery($gallery_id){
    	...
    	$update_data = array(		
							...
							'gallery_sub_title' 				=> $this->input->post( 'gallery_text' ),	
    	...
    	//pr($update_data);
		$condition 		= array('gallery_id' => $gallery_id);
		$this->mcommon->update('master_gallery',$condition,$update_data);
		$log_data 		= array('action' 		=> 'Edit',
								'statement' 	=> "Edited details of the album named - '".$this->input->post( 'gallery_name' )."'",
			...
			if(!empty($imgArry)){
				...
				if($gallery_id){
					...
					$log_data 	= array('action' 		=> 'Add',
										'statement' 	=> "Added a new album named -'".$this->input->post( 'gallery_name' )."'",
										'action_by'		=> $this->session->userdata('user_data'),
										'IP'			=> getClientIP(),
										'id'          	=> $gallery_id,
						  	  			'type'        	=> "Gallery",
	...
	*/
	...
	public function DeleteImage() {
		...
		$gallery_id 	= '';
		$gallery_data 	= $this->mcommon->getRow('gallery_images',array('gallery_img_id' =>$image_id));
		if(!empty($gallery_data)){
			$gallery_id = $gallery_data['gallery_id'];
		}
		$album_name  	= $this->mgallery->getGalleryDataByImgId($image_id);
		$this->db->where('gallery_img_id', $image_id);
   		$this->db->delete('gallery_images');
   		$log_data 	= array('action' 		=> 'Delete',
							'statement' 	=> "Deleted image of the album named -'".$album_name."'",
							'action_by'		=> $this->session->userdata('user_data'),
							'IP'			=> getClientIP(),
							'id'          	=> $gallery_id,
			  	  			'type'        	=> "Gallery",
	...
	public function changeStatus()
	{
		...
		if($return_response){
			$gallery_data 		= $this->mcommon->getRow('master_gallery', array('gallery_id' => $id));
			if(!empty($gallery_data)){
				$album_name   = $gallery_data['gallery_name'];
			}
			if($status == '0'){
			...
			$log_data = array('action' 		=> 'Edit',
							  'statement' 	=> "Made album named -'".$album_name."' ".$changed_status,
							  'action_by'	=> $this->session->userdata('user_data'),
							  'IP'			=> getClientIP(),
							  'id'          => $id,
		  	  				  'type'        => "Gallery",

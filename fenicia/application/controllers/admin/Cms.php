...
class Cms extends MY_Controller {
		...
		if($this->input->post()){
			...
			if($this->form_validation->run()==FALSE){
				...
				if($page_id){
					...
					$log_data = array('id'			=> $page_id,
									  'action' 		=> 'Add',
									  'statement' 	=> "Added a new CMS page named -'".$this->input->post('page_name'). "'",
	...
	public function edit_cms($page_id){
		...
		if($this->input->post()){
			...
			if($this->form_validation->run()==FALSE){
				...
				$udata['status'] = $status;
				$respnse =$this->mcommon->update('cms',array('page_id'=>$page_id),$udata);
				
				if($respnse){
					$log_data = array('id'			=> $page_id,
									  'action' 		=> 'Edit',
									  'statement' 	=> "Edited details of the CMS page named - '".$this->input->post('page_name')."'",
									  'action_by'	=> $this->session->userdata('user_data'),
									  'IP'			=> getClientIP(),
									  'status'		=> '1'
									);
	...
	public function changeStatus()
	{
		...
		if($return_response){
			...
			$page_data 			= $this->mcommon->getRow('cms',array('page_id'=>$id));
		
			if(!empty($page_data)){
				$page_name = $page_data['page_name'];
			}
			$log_data = array('id'			=> $id,
							  'action' 		=> 'Edit',
							  'statement' 	=> "Made CMS page '".$page_name."' ".$changed_status,
							  'action_by'	=> $this->session->userdata('user_data'),
							  'IP'			=> getClientIP(),
							  'status'		=> '1'
							);

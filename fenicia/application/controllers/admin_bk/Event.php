...
class Event extends MY_Controller {
	...
	*/
	...
	public function UpdateEvent($event_id){
		...
		if($this->form_validation->run()==FALSE){
			...
			if(!empty($_FILES["event_img"]["name"][0])){
				...
				if(!empty($imgArry)){
					foreach($imgArry as $img){	
						$instdata_event_media	= array('event_id'=>$event_id,'event_img'  => $img);
			...
			$condition 		= array('event_id' => $event_id);
			$event_id 		= $this->mcommon->update('master_event',$condition,$update_data);
	...
	*/
	...
	public function addPastEventImages(){
		...
		if(!empty($_FILES["past_event_img"]["name"][0])){
    		$imageDetailArray 		= array();
			//echo  "58947";exit;
			$config = array(
			...
			if(!empty($imgArry)){
				foreach($imgArry as $img){	
					$instdata_past_event_media	= array('event_id'=>$event_id,'images'  => $img);
